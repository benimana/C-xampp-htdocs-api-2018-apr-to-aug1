...
//explode (','$jj);
// $wambua_corrupted_employees;
$wam= explode(",", $wambua_corrupted_employees);
$illegal="/[()+=-%<>]/"
 ?>
<table style="border:1px solid black; padding:1px">
  <thead>
    <?php for($i=0;$i<8;$i++):?>
        ...
        echo (preg_replace('/[()0-9>+=\/]+/', '',ucwords(strtolower($wam[$i])))) ; ?></th>
    <?php endfor; ?>
  </thead>
  <tbody>

    <?php for($j=1;$j<(count($wam)-8)/9;$j++):?>
    ...
    if ($i==3||$i==2||$i==5) {
      ...
      $st=str_replace('oo', 'o',preg_replace("/[^a-zA-Z]/",'',ucwords(strtolower($wam[8*$j+$i]))));
    }else if ($i==7) {

      $st=str_replace('oo', 'o',preg_replace('/[@%()>+=\/\^\-]+/', '',ucwords(strtolower($wam[8*$j+$i]))));
    }else if ($i==6) {
    ...
    $st=(preg_replace('/[@%()0-9>+=\/\^\-]+/', '',ucwords(strtolower($wam[8*$j+$i]))));
  }
    echo $st ; ?>
    </td>
    <?php endfor; ?>
